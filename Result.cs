...
using Error = RegistrationService.Aggregate.Error;
using IResult = RegistrationService.Aggregate.IResult;
...
namespace RegistrationService
{
    ...
    public struct Result : IResult, ISerializable
    {
        ...
        private static readonly Func<Exception, string> DefaultTryErrorHandler = (Exception exc) => exc.Message;
        ...
        {
            List<Result> list = results.Where((Result x) => x.IsFailure).ToList();
            if (list.Count == 0)
            {
                return Success();
            }

            return Failure(string.Join(errorMessagesSeparator ?? ErrorMessagesSeparator, list.Select((Result x) => x.Error.Message)));
        }

        public static Result Combine<T>(IEnumerable<Result<T>> results, string errorMessagesSeparator = null)
        {
            return Combine(results.Select<Result<T>, Result>((Result<T> result) => result), errorMessagesSeparator);
        }
